...
export default class EffectReview extends Component {
  ...
  backForwardButtons(id, key) {
    return (
      <span>
        <span className="arrow" onClick={this.prevOption.bind(this, id, key)}>&#8592;</span>
        <span className="arrow" onClick={this.nextOption.bind(this, id, key)}>&#8594;</span>
      </span>
    );
  }
  ...
  generateBackgroundSelects() {
    return (
      <span style={{"margin":"0px 5px 0 0"}}>
        <select name={"background-selection"} id={"background-selection"} onChange={this.updateState.bind(this, 'background-selection', 'background')}>>
  ...
  generateVisualEffectsSelects() {
    return (
      <span style={{"margin":"0px 5px 0 0"}}>
        <select name={"visual-effect"} id={"visual-effect"} onChange={this.updateState.bind(this, 'visual-effect', 'visualEffect')}>>
          {VisualEffects["visual-effects"].map((key) => <option name={"visual-effect"} value={key}>{key}</option>)}
        </select>
